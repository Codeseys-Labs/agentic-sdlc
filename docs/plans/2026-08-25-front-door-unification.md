...
| Refusals | `missing-selector` (2) · `wildcard-selector` (2) · `mode-forbidden-at-project-scope` (2) · `project-flag-without-project-scope` (2) · `unresolvable-project-root` · `unsafe-node` · `not-a-git-project` · `forbidden-root` · `untrusted-root` · `agent-not-yet-receipted` (pre-WX only) · `payload-manifest-mismatch` · `marketplace-overlap` (user+claude only; project scope emits a named `overlap:` advisory line and blocks nothing — ratified, §8 D5) · `foreign-destination` (~~per entry; all-or-nothing before any write~~ — **DATED CORRECTION 2026-08-26, W6: the implementation is the doctrine of record.** An occupied foreign or operator-drifted destination is NOT an all-or-nothing pre-write refusal and never was: it is a per-entry **preserve-and-name at exit 0**, with the entry left byte-identical, no ledger row written for it, and the receipt's own inventory row recording `prestate: foreign` / `disposition: preserved`. This predates W5 and is uniform across entry kinds — it is `assert_safe_collection`-bounded classification plus the installer's preservation rules, which "preserved and reported" has meant since the ownership model was written, not a project-scope novelty. The ratified sentence conflated two different controls: `payload-manifest-mismatch`, which really is all-or-nothing before any write because a payload that fails its own manifest has no admissible identity, and an occupied destination, which is one entry's classification and cannot speak for the other twenty-six. Measured on the landed tree: a project install against a repository holding an operator-edited `.claude/workflows/sdlc-wave-scout.js` exits 0, leaves the bytes unchanged, writes no ledger row for it, and reports `preserved and named, never adopted` — `tests/test_project_scope_acceptance.py:441-452`. Nothing here weakens a refusal; it names the one that exists) · `legacy-pointer-ambiguity` (§3.5). |
...
| **W3a** | Front door, grammar half: top-level verb table in `bin/ccodex` + `ccodex_sdlc.py` grammar (`--scope`/`--agent`), retired-spelling refusal arms (§2.4), the `require_toolchain` exit-class stance (below), **re-derivation of the release gate's mutation patch** (audit B5: the patch is textually anchored to the old `sdlc)` arm with six lines of context, and its own preamble names the re-derive recipe — W3a owns the patch and its preamble), and the new verbs' `SEAM_CASES` entries (the harness fails the gate on an uncased verb by construction). | `bin/ccodex`, `ccodex_sdlc.py` (grammar), `.github/mutations/restore-v0.7.4-uv-run-sdlc-route.patch`, `tests/seam_harness.py` cases, tests | W2, WX, D3+D4 (single dispatcher before the verb-table reshape) | check green; retired spellings answer with messages naming the replacement; **the re-derived mutation patch still turns the surviving route red** via `release:smoke --expect-refusal` naming `runtime-admission-refused` **EXECUTED 2026-08-25 (`e62277f`, `2a0b720`), with corrections:** (1) **`status` COLLIDES with this command's existing gateway supervision verb** and §2.1 does not price it — resolved by presence of a lifecycle selector (`--scope`/`--agent`/`--project`), never by a shape heuristic, with both directions mutation-proven and the gateway-route test restored on a stub-launcher tree; (2) `inspect` is RETIRED rather than mapped, since §2.4 lists it against two replacements and the ratified table has six verbs — the digest-pinned report policy is untouched because `command_verbs` is a membership check over an admitted SET; (3) §2.1's `--mode`, `--dry-run`, and `--scope project` are PARSED and refused at exit 3 by name (`mode-not-yet-wired`, `dry-run-not-yet-wired`, `project-scope-not-yet-wired`), because no wave wires them here and both alternatives — silent drop, or exit 2 — are dishonest; (4) `--agent` is the operator spelling and `--host` remains the module ABI, built in ONE place and pinned by a test, because the four per-verb modules are other waves' files — the residual is that those modules still print `error: ccodex sdlc <verb> …`, which the seam cases assert as emitted rather than as correct; (5) **the retirement removes the only dispatcher route to the ledger substrate**, so `ccodex install` refuses without an acquired candidate until W3b and `mise run lifecycle:install -- --agent claude` is the interim path — the smoke manifest's installer terminal-line case went with it, and that string stays pinned in-process; (6) the `bundle:*` task family renamed to `lifecycle:*`, which reached three surfaces no test or validator checks (`contributor:setup`'s `depends`, `run_all_hosts.py`'s derived task name, `real-key-testing.yml`) and would have failed at first use with `check` green. |
...
| **W4** | Project scope: §2.2 ladder with the detector extracted to a shared module (consumed by both the installer and `offline-inspect.py`), copy forcing, §3.7 adoption with the N4 printed line, project-root wiring for the W2 uninstall path, the B6 remedies (`doctor` `orphaned-root`; records-only uninstall of a nonexistent root), project-scope `overlap:` advisory line (§8 D5). | new shared detector module, `install_skill_bundle.py`, `ccodex_sdlc_install.py`/`_update`/`_uninstall` (scope plumbing), `offline-inspect.py` (consume shared module), tests | W1, W3b (shared `ccodex_sdlc_install.py` — ordering edge, R1; W3b landing first also lets W4's transcripts exercise release-root payloads, not only checkouts) | check green; gh #11 acceptance 5–8 in throwaway repos (three live pointers; A's uninstall leaves B and user byte-identical; `find <repo>/.claude -type l` empty; forbidden-root and unsafe-node refusals with `find <repo> -newer <marker>` empty — pinned to the **target root**, audit W-h) **EXECUTED 2026-08-26 (3a7db10), with corrections:** the file list was short by four (ccodex_sdlc.py, host_planes.py, the read-report policy + its pin); §4.3's window premise is FALSE — the workflows manager writes NO installer ledger row, so a project install would have ADOPTED an activated copy into dual authority; the honest interim is PROJECT_DEFERRED_KINDS=("workflow",) printed on every project run until W5 deletes the manager; §3.7's adoption receipt row is owned+preserved with null published mode (the plan never said); §2.2 item 6's records-only retirement REMOVES the pointer while ordinary retirement keeps it (asymmetry now stated in code); codex at project scope refuses project-scope-unsupported-for-agent (layout decision exceeds evidence — recorded in the plane table); §2.3's "W1 deletes --claude-home" was never true (seed 3605); status body narrowing needs a report-policy scope dimension (seed 95e6); the WX collision class recurred one level down and is closed per-root via scope_token(). |
| **W5** | Subsume the workflows manager: delete `manage_claude_workflows.py`, its tasks, tests; session-snapshot sentence on project-scope verbs; `doctor` names the orphan store. | `manage_claude_workflows.py` (delete), `mise.toml`, `bin/ccodex` (if any task alias), tests | W4 | check green; `grep -rn 'manage_claude_workflows\|agentic-sdlc-claude-workflows' scripts/ tests/ mise.toml` = 0; project install leaves `<repo>/.claude/workflows/<name>.js` as a copy and prints the sentence **EXECUTED 2026-08-25 (818bf09), annotated 2026-08-26 by W6, with corrections:** (1) **the one-grep check was unsatisfiable together with §4.3** and is re-cut as two: `manage_claude_workflows` = 0 across `scripts/`, `tests/`, `mise.toml` (met exactly), while `agentic-sdlc-claude-workflows` legitimately SURVIVES three times as the store-naming literal — `scripts/ccodex_sdlc.py`'s `ORPHANED_WORKFLOW_RECEIPTS_STORE` plus two `tests/seam_harness.py` fixture sites — because §4.3 requires `doctor` to NAME that store, and a reader cannot name a directory whose name it may not spell. Same shape as `RETIRED_OPERATOR_TOOLS_STORE` from gh #10 phase 4, and the same *unsatisfiability* rather than a carve-out: gh #8 acceptance 7's `grep -rn 'operator-tools\|XDG_BIN_HOME\|HostPreconditionError' scripts/ bin/ mise.toml` measures **24** on this tree (2026-08-26) across five files, because the store the reader must name and the retirement prose that carries the manual remedy both spell the retired plane. Seed agentic-sdlc-32a2 already records its own criterion 2 as unsatisfiable by design, with the import-freedom ratchet enforcing the stronger executable-source-zero form; a zero-grep over a store name is only ever satisfiable when nothing reports the store. (2) **The owned-file list was short by thirteen paths**, none of them tests: `AGENTS.md`, `README.md`, `commands/sdlc-init.md`, `plugin/commands/sdlc-init.md` (the drift-checked mirror of the previous, so twelve distinct sources), `policy/ccodex-sdlc-read-report.v1.json` (`finding_components` gained `claude-workflows`), `scripts/validate_bundle.py` (that policy's digest pin plus the three deleted task names), `scripts/ccodex_sdlc.py` (the orphan-store finding), `scripts/ccodex_sdlc_host_planes.py` (`project_session_note`), `scripts/ccodex_sdlc_install.py`, `scripts/ccodex_sdlc_update.py`, `scripts/ccodex_sdlc_uninstall.py` (the three note readers), `scripts/install_skill_bundle.py` (`PROJECT_DEFERRED_KINDS` deleted), and `scripts/distribution_activation_receipt.py` (one comment naming the deleted module). `bin/ccodex` was listed and NOT touched — the "if any task alias" hedge resolved to none. The pattern across this train is now three-for-three (W4 short by four, W5 by thirteen): a wave that deletes a plane owns every file that NAMED it, and the file lists are written from the plane's own surface rather than from its consumers. (3) **§4.2's "every completed project-scope verb prints the session-snapshot sentence, carried over verbatim" was one-third true before this wave, on both halves.** Pre-W5 (`9e75c15`) there were ZERO readers of any shared sentence: `install` printed a reworded local copy of its own (`ccodex_sdlc_install.py:2920`), `update` and `uninstall` printed nothing, and the only verbatim copies lived in the manager that was about to be deleted. Landed: one canonical `HostPlane.project_session_note`, read by the three MUTATING verbs, with a test asserting no verb module carries a second copy of the sentence. The read verb stays silent, and that is the deleted module's own precedent rather than an omission — the manager appended its note in `_activate` and `_deactivate` and not in `status`, because a read makes no change and so has no session to become visible in. (4) **§4.3's remedy text as drafted exceeded the reader's own `MAX_FINDING_MESSAGE_CHARS` (240)** and would have been truncated at exactly the wrong end, losing the clause that says what to verify BEFORE deleting evidence; it landed at 222 characters with `tests/test_ccodex_sdlc.py:987` bounding every retired-store remedy against the constant, so a future reword that overruns fails the gate instead of silently shortening a recipe. |
| **W6** | Decision debt: ADR-0021 amendments (§5), ADR-0028 registry agreement, AGENTS.md/README task-list re-diff, clean-host project-scope transcript under `docs/evidence/`, and the train's **terminal check**: `doctor --json` store count equals the tree's store count (gh #8 acceptance 9 — only decidable here, per §2.3/audit B4). | docs + the terminal check | W5, D3+D4 | check green (docs gate: broken references, secrets); transcript committed; store-count check recorded **EXECUTED 2026-08-26, with corrections:** (1) **"docs + the terminal check" was wrong about the terminal check**, which is the largest correction in this row. Acceptance 9 was not a check waiting to be *run*; it was a check that FAILED, and satisfying it needed the reader's fourth v1 policy edit of this train. Measured before the change on a fixture host carrying all six stores, `doctor --json` named **three**: `bundle.state_paths` for the ownership ledger and one `foreign-state` finding each for the two leftovers, while the receipt-and-pointer plane (`<state>/agentic-sdlc/`), the statusline receipt store, and the hook receipt store were named nowhere in the report. Landed: one `STATE_STORES` roster in `scripts/ccodex_sdlc.py` naming all six with a component, a `live`/`retired` kind, and a per-store `present`/`absent`/`unreadable` verdict; a new `state_stores` top-level report field on **every** read verb rather than on `doctor` alone, because a whole-box fact conditional on the verb would be a representable illegal state in an exact-key report shape; `RETIRED_STORES` demoted to a filter over that one roster; and `validate_state_stores` refusing a report that names fewer stores than the roster, so the count is binding rather than advisory. (2) **The count is derived independently, or it would be the reader grading its own homework.** `tests/test_ccodex_sdlc_state_stores.py` re-derives the store-name set from the owning modules' own source by AST walk — every state-store-shaped string literal that is a `pathlib` join operand or a member of a `*_SEGMENTS`/`*_STORE`/`*_DIRECTORY`/`*_PLANE` constant across `scripts/*.py` — and asserts set equality with the roster in both directions, so a seventh store fails the gate until the reader names it and a roster row for a store nothing writes fails too. Mutation-proven four ways (drop a roster row · add a phantom row · give a live row a remedy · truncate the emitted roster), each red, restored green. It is a syntax walk and not a `grep` for the good reason that a text scan also matches seed ids inside docstrings, the published statusline command's basename, and a tempdir prefix; the two remaining `agentic-sdlc` hits are `skills/agentic-sdlc/…` joins that collapse onto a name that IS a store. (3) **`policy/ccodex-sdlc-read-report.v2.json` is DELETED** with its digest pin (conductor ruling): nothing read it — the reader opens the v1 document alone — and by this wave it had drifted three reviewed v1 edits behind (no `orphaned-root`, no `pointer-outlived-root`, no `claude-workflows`) while still carrying the `operator_tools` top-level field gh #10 phase 4 deleted, so wiring it would have refused every read verb. That closes the `read-report.v2` item of seed agentic-sdlc-32a2's phase-5 sweep; the `release-contract.v1.json` `channels` block remains open in that seed. `validate_bundle.py`'s own docstring claiming both documents are parsed on every invocation went with it — it was false for v2 at every commit, which is precisely how a pinned document kept a reader-less schema alive. (4) **§2.3's `foreign-destination` sentence is corrected in place** as a dated correction rather than re-litigated: the landed behavior is per-entry preserve-and-name at exit 0, not an all-or-nothing pre-write refusal. (5) **`tests/test_product_spec_contract.py` blocked this wave's own doc debt** (seed agentic-sdlc-a010) and is relaxed on both axes: `stray_verbs` now scans only command spans (inline code and fenced blocks), so the retirement sentence "`ccodex sdlc` is retired" no longer reports a stray verb `is`, and provenance/enumeration are scoped to the block's `ccodex sdlc` lines, so Implementation Decision 91 can gain a line naming the ratified surface without the pre-ratification brief being asked to source it. Found while fixing it: the fence pattern was anchored at column 0 while ID 91's fence is indented four spaces inside a numbered list, so the ONE block the closure exists to police was never scanned — that hole is closed and pinned by its own case. Teeth re-proven on the real spec file: the withdrawn eighth verb planted in ID 91's fence goes red, a dropped brief-sourced line goes red, the ratified line passes, and the file was restored byte-identical. (6) **Recording the ratified verb surface in the product spec itself is NOT done and is deliberately left open**: the spec's own preamble makes it a product-decision artifact that "does not supersede" and requires a return to product decision work, so a documentation wave unblocking the predicate is the correct half to land here. (7) **The clean-host transcript is `docs/evidence/2026-08-26-project-scope-clean-host.md`**, executed against the built archive of `438ce67` placed as an acquisition candidate rather than against the checkout, on fixture planes sharing nothing with this host's real Claude home or state root. It surfaced three defects the offline suite could not: the uninstall's SUCCESS banner prints the retired `ccodex sdlc uninstall` spelling to an operator who typed `ccodex uninstall` (`ccodex_sdlc_uninstall.py:1453`; `install`'s banner is correct, so the two verbs disagree), seven empty directories survive the retirement unnamed in the output while `git status` reads clean because git does not track empty directories, and `doctor` prints `state store [bundle/live]: present` beside `bundle: absent` in one report on a host holding 29 project rows (the already-filed status-narrowing gap, seed agentic-sdlc-95e6, now contradicting itself in front of an operator). None is fixed here: two are other waves' files and the third is a decision rather than a bug fix. |
...
commit, which is exactly the discipline the harness exists to force. **EXECUTED**: the message
prefix moved with the class (`error:` → `refused:`), because the other exit-3 refusal in that
function already spelled it that way and two prefixes for one class is the same defect one level
down. `test_lifecycle_exit_conformance.py` gained a case pinning the pair, which only became
provable once that suite drove the dispatcher as a process.
