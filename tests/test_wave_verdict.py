...
# `WaveCase` builds every fixture by RUNNING `wave-journal.py init` first, and that publish refuses
# (by name, at exit 3) on a host that lacks this syscall -- so `WaveCase` itself is skipped there
# rather than every fixture-building test failing downstream. On POSIX this is a pure SYMBOL probe,
# never `sys.platform`, so it can only be false on a host that genuinely lacks the syscall; glibc 2.28+
# always exports it, so this is never false on the Linux CI runner this suite must stay green on. The
# `os.name` term answers the strictly earlier question of whether `ctypes.CDLL(None)` may be CALLED at
# all: on native Windows it may not (3.12's `CDLL.__init__` takes its `_os.name == "nt"` branch and
# evaluates `'/' in name` with `name=None`, a TypeError), and at module scope that would be a loader
# traceback for this whole file on the windows CI leg instead of a named skip.
# `test_the_probe_returns_non_none_on_this_linux_host` is this constant's own positive control.
...
class RenameAt2CapabilityTests(unittest.TestCase):
    """The capability probe `WaveCase` is skipped on. Deliberately NOT a `WaveCase`: this class's
    whole point is to run even when `_HAS_RENAMEAT2` is false and every `WaveCase` here is skipped.
    `wave-journal.py`'s own refusal at the missing syscall is covered directly by
    `tests/test_wave_journal.py`'s `RenameAt2CapabilityTests`; this is only the probe this module
    reuses to decide whether to build fixtures with it at all.
    """
    ...
    def test_the_probe_returns_non_none_on_this_linux_host(self) -> None:
        """POSITIVE CONTROL: without this, `_HAS_RENAMEAT2` gating `WaveCase` above would be silently
        vacuous -- true on every host, including the ubuntu CI runner this suite must stay green on --
        and nothing would ever fail to reveal it.
