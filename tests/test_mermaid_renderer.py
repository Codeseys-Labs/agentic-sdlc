...
class MermaidRendererTests(unittest.TestCase):
    ...
    def test_renderer_passes_absolute_child_deadlines(self) -> None:
        policy = renderer.load_policy(POLICY_PATH)
        with tempfile.TemporaryDirectory() as temp, mock.patch.object(renderer, "_runtime_receipt", return_value={"node_executable": "/bin/true"}), mock.patch.object(renderer, "_cache_digest", return_value=policy["browser"]["cache_tree_sha256"]), mock.patch.object(renderer, "_sha256", return_value=policy["browser"]["executable_sha256"]), mock.patch.object(renderer, "resolve_node_bin_shim", return_value=Path("/bin/true")), mock.patch.object(renderer, "write_owner_configs", return_value=(Path(temp) / "mermaid.json", Path(temp) / "puppeteer.json")), mock.patch.object(renderer, "_run_child") as child, mock.patch.object(renderer.time, "monotonic", side_effect=[10.0, 12.0]):
            root = Path(temp)
            ...
            with mock.patch.object(renderer.time, "monotonic", side_effect=[10.0, 10.0, 12.0, 12.0]):
